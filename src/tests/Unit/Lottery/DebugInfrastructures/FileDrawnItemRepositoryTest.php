...
class FileDrawnItemRepositoryTest extends TestCase
{
    ...
    {
        $boxId = new BoxId(str_repeat('a', 26));
        $itemId1 = new ItemId(str_repeat('b', 26));
        $drawnAt1 = new DrawnAt(new DateTimeImmutable('2025-06-14 14:26:30'));

        $itemId2 = new ItemId(str_repeat('c', 26));
        $drawnAt2 = new DrawnAt(new DateTimeImmutable('2025-06-15 14:26:30'));

        $this->store->shouldReceive('getAll')
            ->with('/drawn-item.dat')
            ->andReturn([
                $boxId->value . '-' . $itemId1->value . '-' . $drawnAt1->value->format('YmdHis') => new DrawnItem($boxId, $itemId1, $drawnAt1),
                $boxId->value . '-' . $itemId2->value . '-' . $drawnAt2->value->format('YmdHis') => new DrawnItem($boxId, $itemId2, $drawnAt2),
            ])
            ->once();
        ...
        $this->assertCount(1, $result);
        $this->assertSame($boxId->value, $result[0]->boxId->value);
        $this->assertSame($itemId2->value, $result[0]->itemId->value);
